...
class MindfullExerciseProvider extends ChangeNotifier {
  ...
  List<MindFulnessExercise> getAllMindfullExercises(){
  ...
  void searchMindfullExercise(String query) {
    if(query.isEmpty){
      ...
      mindfullExercise = _allMindfullExercises.where((exercise) => 
      exercise.name.toLowerCase().contains(query.toLowerCase())).toList();
    }
    notifyListeners();
  }
}